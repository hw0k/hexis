...
- **RED:** Run the scenario in a fresh Claude Code session with NO `hexis` skills loaded. The scenario **passes RED** if the agent violates the rule (confirming the test catches a real failure mode).
- **GREEN:** Run the same scenario in a session with `hexis:commit-principles` loaded. The scenario **passes GREEN** if the agent complies with the rule.
...
4. **GREEN phase:** In a new fresh session, load `hexis:commit-principles`. Paste the same prompt.
