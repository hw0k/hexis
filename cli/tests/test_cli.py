...
def test_status_read_needs_verify_plain(tmp_path):
    ...
    assert "PLAN TASKS:" not in result.output
...
def test_status_update_keeps_in_progress_when_plan_frontmatter_is_not_done(tmp_path):
    specs_dir = tmp_path / "docs" / "specs"
    plans_dir = tmp_path / "docs" / "plans"
    specs_dir.mkdir(parents=True)
    plans_dir.mkdir(parents=True)
    ...
    plan_path.write_text(
        "---\nissue: 5\nstatus: READY_TO_IMPLEMENT\nlinked_spec: docs/specs/spec.md\n---\n\n- [x] Body checkbox that should not matter\n"
    )
